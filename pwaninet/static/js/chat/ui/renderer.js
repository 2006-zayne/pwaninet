...
export class MessageRenderer {
  ...
  init(config) {
    ...
    appendMessage(message) {
        const viewItem = this.buildView([this.eventBus.off ? this.getMessages() : [message])[0];
        const messageElement = this.createMessageElement({
            ...message,
            isOwn: message.sender_id === this.currentUserId,
            isConsecutive: this.isConsecutive(message),
            isLastSent: message.sender_id === this.currentUserId
        });
    ...
    isConsecutive(message) {
        // This would need access to previous message
