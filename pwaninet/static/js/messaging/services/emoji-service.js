...
export class EmojiService {
  ...
  setupEventListeners() {
    ...
    eventBus.on('EMOJI_PICKER_TOGGLE', (isOpen) => {
      this.isOpen = isOpen;
    });

    // Listen for emoji insertion requests
    eventBus.on('EMOJI_INSERT_REQUEST', (emoji) => {
  ...
  insertEmoji(emoji) {
    ...
    eventBus.emit('EMOJI_INSERTED', emoji);
  }
  ...
  openPicker() {
    ...
    this.isOpen = true;
    eventBus.emit('EMOJI_PICKER_TOGGLE', true);
  }
  ...
  closePicker() {
    ...
    this.isOpen = false;
    eventBus.emit('EMOJI_PICKER_TOGGLE', false);
  }
