...
export class OptimisticUpdateService {
  ...
  confirmMessage(serverMessage) {
    // Find pending message by content/timestamp/sender
    let tempId = null;
    ...
    for (const [id, data] of this.pendingUpdates.entries()) {
      ...
      const optimisticTime = new Date(data.optimisticMessage.created_at).getTime();
      const timeDiff = Math.abs(serverTime - optimisticTime);

      // Match if content matches and time difference is within 10 seconds
      // OR if it's our own message and time difference is within 10 seconds
      if (
        (data.optimisticMessage.content === serverMessage.content && timeDiff < 10000) ||
        (serverMessage.sender_id === store.currentUserId && timeDiff < 10000)
      ) {
        tempId = id;
        break;
      }
    ...
    if (tempId) {
      ...
      // If no matching optimistic message found, this might be a message from another device
      // Add it as a new message
      console.log('No matching optimistic message found, adding as new message');
      eventBus.emit(EVENTS.MESSAGE_NEW, serverMessage);
    }
  }
