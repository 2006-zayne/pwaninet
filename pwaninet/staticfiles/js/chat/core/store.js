...
export class Store {
  ...
  init(config) {
    ...
    this.currentUserPic = config.currentUserPic || null;
  ...
  /**
   * Replace message in state (for optimistic updates)
   * @param {string} oldMessageId - Old message ID (temp ID)
  ...
  replaceMessage(oldMessageId, newMessage) {
    ...
    // Remove old ID from processed set
    this.processedMessageIds.delete(oldMessageId);

    // Replace message
    this.messages[messageIndex] = newMessage;

    // Add new ID to processed set
